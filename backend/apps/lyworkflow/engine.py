...
class FlowEngine:
    ...
    def _handle_approve_pass(self, current_task: WorkflowTask):
        ...
        current_level_order = current_task.level_order if hasattr(current_task, 'level_order') else float(current_task.step_order)
        ...
        if current_step.approver_type == 6 and current_step.multi_level_config:
            # 多级审批：检查是否还有下一个层级
            next_level_order = self._get_next_multi_level_order(current_step, current_level_order)
            
            if next_level_order is not None:
                # 有下一个层级，流转到下一层级
                # current_step 保持为整数（原始步骤顺序），不更新
                
                self._create_tasks_for_step(current_step_order, next_level_order)  # 传入下一层级的 level_order
                self._create_log('next_level', f'流转到多级审批下一层级')
                return
        ...
        if next_step:
            ...
            self._create_tasks_for_step(next_step.step_order, float(next_step.step_order))  # 普通步骤 level_order = step_order
            self._create_log('next_step', f'流转到下一步骤: {next_step.step_name}')
        else:
            # 所有步骤完成，流程通过
            instance.status = 2  # 已通过
            # current_step 保持为总步骤数（整数）
            instance.current_step = instance.total_steps
            instance.save()
            self._create_log('complete', '流程已完成')
    
    def _get_next_multi_level_order(self, step: WorkflowStep, current_level_order: float):
        """
        获取多级审批的下一个层级顺序
        
        Args:
            step: 多级审批步骤
            current_level_order: 当前层级的 step_order（小数）
            
        Returns:
            下一个层级的 step_order，如果没有则返回 None
        """
        if not step.multi_level_config:
            return None
        
        # 从当前层级顺序中提取整数部分（原始 step_order）
        base_step_order = int(current_level_order)
        
        # 计算当前是第几个层级
        current_level_idx = int(round((current_level_order - base_step_order) / 0.1))
        
        # 检查是否还有下一个层级
        if current_level_idx + 1 < len(step.multi_level_config):
            # 有下一个层级，返回其 step_order
            next_level_order = base_step_order + ((current_level_idx + 1) * 0.1)
    ...
    def _create_tasks_for_step(self, step_order: int, level_order: float = None):
        """为指定步骤创建审批任务（支持多级审批展开）
        
        Args:
            step_order: 步骤顺序（整数，表示原始步骤顺序）
            level_order: 层级顺序（浮点数，多级审批时使用，如 1.0, 1.1, 1.2；普通步骤等于 step_order）
        """
        try:
            # 如果未传入 level_order，默认等于 step_order
            if level_order is None:
                level_order = float(step_order)
            ...
            if step.approver_type == 6 and step.multi_level_config:
                self._create_multi_level_tasks(step, level_order)
            else:
                # 普通步骤，直接创建任务
                ...
                for user in approver_users:
                    WorkflowTask.objects.create(
                        ...
                        level_order=level_order,  # 浮点数（普通步骤等于 step_order）
                        approver=user
                    )
    ...
    def _create_multi_level_tasks(self, step: WorkflowStep, target_level_order: float):
        """
        为多级审批创建任务
        
        Args:
            step: 多级审批步骤
            target_level_order: 目标层级顺序（如 1.0, 1.1, 1.2）
        """
        if not step.multi_level_config:
        ...
        for idx, level in enumerate(step.multi_level_config):
            # 计算该层级的 level_order（与序列化器中的逻辑一致）
            level_order = step.step_order + (idx * 0.1)
            
            # 只处理当前应该审批的层级
            # 使用容差比较浮点数
            if abs(level_order - target_level_order) < 0.01:
                ...
                for user in approver_users:
                    WorkflowTask.objects.create(
                        ...
                        level_order=level_order,  # 浮点数，表示层级顺序
                        approver=user,
                        # 可以在 remark 中记录这是哪一级
